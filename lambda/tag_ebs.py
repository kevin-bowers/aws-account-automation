...
def process_AttachVolume(event, volume_id, username):
    ...
    logger.info("user: " + username + " attached Volume " + volume_id + " to " + instance_id + " as " +
                event['detail']['responseElements']['device'])
    tag_volume("attached", volume_id, username, instance_id, event['detail']['responseElements']['device'],
               event['detail']['eventTime'])
    # Remove detachment info
    try:
        response = client.delete_tags(Resources=[volume_id],
                                      Tags=[{'Key': TAG_PREFIX + 'detached_by'}, {'Key': TAG_PREFIX + 'detached_date'}])
    except ClientError as e:
...
def process_RunInstances(event, username):
    ...
    for i in event['detail']['responseElements']['instancesSet']['items']:
        ...
        for device in instance['BlockDeviceMappings']:
            tag_volume("attached", device['Ebs']['VolumeId'], username, i['instanceId'], device['DeviceName'],
                       device['Ebs']['AttachTime'].isoformat())
...
def process_on_schedule():
    ...
    for v in response['Volumes']:
        ...
        try:
            ...
            if a['InstanceId'] not in t[TAG_PREFIX + 'attached_to_instance'] or a['Device'] not in t[
                TAG_PREFIX + 'attached_to_instance']:
                try:
